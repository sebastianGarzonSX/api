...
const openCors = cors()
trackingRouter.options('/pageview', openCors)
...
function hashIp(ip: string): string {
  return crypto.createHash('sha256').update(ip + (process.env.IP_SALT ?? 'lp-track')).digest('hex').slice(0, 16)
}

// ── POST /api/tracking/pageview ── PÚBLICO (sin auth) ────────────────────────
trackingRouter.post('/pageview', openCors, async (req, res) => {
  try {
    ...
    const { tag, page_url, referrer } = req.body as {
      tag?: string
      page_url?: string
      referrer?: string
    }
    ...
    const { error } = await supabaseAdmin.from('lp_pageviews').insert({
      tag:        tag.trim(),
...
trackingRouter.get('/lp-views', authenticate, async (req, res) => {
  try {
...
trackingRouter.get('/snippet', authenticate, async (req, res) => {
  const tag = (req.query['tag'] as string | undefined) ?? null
  if (!tag) {
    res.status(400).json({ error: 'tag requerido', status: 400 })
    return

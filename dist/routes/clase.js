...
exports.claseRouter = (0, express_1.Router)();
const MONTH_MAP = {
...
function tagToClassDate(tag, ref = new Date()) {
    const m = tag.toLowerCase().match(/clase\s+(\d{1,2})\/([a-záéíóú]+)/);
    if (!m)
        return null;
    const month = MONTH_MAP[m[2]];
...
exports.claseRouter.get('/report', auth_js_1.authenticate, async (req, res) => {
    try {
        ...
        const { data, error } = await supabase_js_1.supabaseAdmin.rpc('get_clase_report', {
            p_class_tag: tag,
            p_since: since,
            p_until: until,
        });
        ...
        const { count: lpCount } = await supabase_js_1.supabaseAdmin
            .from('lp_pageviews')
...
exports.claseRouter.get('/lp-views', auth_js_1.authenticate, async (req, res) => {
    try {
        const tag = req.query['tag'] ?? null;
        if (!tag) {
...
exports.claseRouter.post('/lp-views', auth_js_1.authenticate, (0, requireRole_js_1.requireRole)('admin'), async (req, res) => {
    try {
        const { tag, lp_views } = req.body;
        if (!tag?.trim()) {
            res.status(400).json({ error: 'tag requerido', status: 400 });
            return;
        }
        if (lp_views === undefined || lp_views === null || Number.isNaN(Number(lp_views)) || Number(lp_views) < 0) {
            res.status(400).json({ error: 'lp_views debe ser >= 0', status: 400 });
            return;
        }
...
exports.claseRouter.get('/summary', auth_js_1.authenticate, async (req, res) => {
    try {
        const now = new Date();
        const until = req.query['until'] ?? now.toISOString().slice(0, 10);
        const since = req.query['since']
            ?? new Date(now.getTime() - 120 * 86_400_000).toISOString().slice(0, 10);
        const { data, error } = await supabase_js_1.supabaseAdmin.rpc('get_clases_summary', {
            p_since: since,
            p_until: until,
        });
        if (error) {
...
exports.claseRouter.get('/meta', auth_js_1.authenticate, async (req, res) => {
    try {
        const now = new Date();
        const until = req.query['until'] ?? now.toISOString().slice(0, 10);
        const since = req.query['since']
...
exports.claseRouter.get('/pixel-events', auth_js_1.authenticate, async (req, res) => {
    try {
        const tag = req.query['tag']?.trim() || null;
        const accountId = req.query['account_id']?.trim() || null;
        const now = new Date();
        const reqUntil = req.query['until'] ?? now.toISOString().slice(0, 10);
        const reqSince = req.query['since']
            ?? new Date(now.getTime() - 30 * 86_400_000).toISOString().slice(0, 10);
        ...
        const all = await (0, meta_js_1.fetchPixelEvents)(since, until, accountId);
        // Filtrar a campañas de Clase en Vivo
        const claseRows = all.filter((r) => (r.campaign_name ?? '').toLowerCase().includes('clase sem'));
        ...
        const TARGETS = [
            ...
            },
            {
                ...
                matchers: (t) => t === 'complete_registration' ||
                    t.endsWith('.fb_pixel_complete_registration'),
            },
            {
                key: 'contact',
                label: 'Contact',
                matchers: (t) => t === 'contact_total' || t === 'contact' || t.endsWith('.fb_pixel_contact'),
            },
            {
                key: 'wa_group_click',
        ...
        for (const t of TARGETS)
            totals[t.key] = { count: 0, cost_per: 0, spend_share: 0 };
        let totalSpend = 0;
...
exports.claseRouter.get('/calendar-config', auth_js_1.authenticate, async (_req, res) => {
    try {
        const { data, error } = await supabase_js_1.supabaseAdmin
            .from('app_settings')
            .select('value')
...
exports.claseRouter.post('/calendar-config', auth_js_1.authenticate, (0, requireRole_js_1.requireRole)('admin'), async (req, res) => {
    try {
        const { calendar_id } = req.body;
        if (!calendar_id?.trim()) {
            res.status(400).json({ error: 'calendar_id requerido', status: 400 });
            return;
        }
        const { error } = await supabase_js_1.supabaseAdmin
            .from('app_settings')
...
exports.claseRouter.get('/appointments', auth_js_1.authenticate, async (req, res) => {
    try {
        const now = new Date();
        const tag = req.query['tag']?.trim() || null;
        const reqUntil = req.query['until'] ?? now.toISOString().slice(0, 10);
        ...
        const { data: cfg, error: cfgError } = await supabase_js_1.supabaseAdmin
            .from('app_settings')
            .select('value')
            .eq('key', CALENDAR_KEY)
            .maybeSingle();
        if (cfgError) {
            res.status(500).json({ error: cfgError.message, status: 500 });
            return;
        }
        if (!cfg?.value) {
        ...
        const appointments = await (0, ghl_js_1.fetchCalendarAppointments)(cfg.value, since, until);
        // Si hay tag, obtener los contact_ids de leads que tienen ese tag
        let allowedContactIds = null;
        if (tag) {
            const { data: tagLeads, error: tagErr } = await supabase_js_1.supabaseAdmin
                .from('leads')
            ...
            allowedContactIds = new Set((tagLeads ?? [])
        ...
        if (contactIds.length > 0) {
            const { data: leads } = await supabase_js_1.supabaseAdmin
                .from('leads')
                .select('ghl_contact_id, name, email')
                .in('ghl_contact_id', contactIds);
            contactsById = new Map((leads ?? []).map(l => [l.ghl_contact_id, { name: l.name, email: l.email }]));
        }

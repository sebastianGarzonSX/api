...
exports.adsmanagerRouter = void 0;
const express_1 = require("express");
const crypto_1 = require("crypto");
const supabase_js_1 = require("../lib/supabase.js");
const auth_js_1 = require("../middleware/auth.js");
exports.adsmanagerRouter = (0, express_1.Router)();
...
function appSecretProof(token) {
    if (!ENV_SECRET)
        return '';
    return (0, crypto_1.createHmac)('sha256', ENV_SECRET).update(token).digest('hex');
}
async function metaFetch(path, token, extraParams = {}) {
...
async function metaFetchAll(path, token, params) {
    const all = [];
    const proof = appSecretProof(token);
    ...
    let url = `${META_BASE}${path}?${baseParams}`;
    while (url) {
        const ctrl = new AbortController();
        const t = setTimeout(() => ctrl.abort(), META_TIMEOUT_MS);
        const res = await fetch(url, { signal: ctrl.signal }).finally(() => clearTimeout(t));
...
async function getUserToken(userId) {
    ...
    const userToken = data?.meta_token;
...
function parseDateRange(date, since, until) {
    if (since && until)
        return { since, until };
    const now = new Date();
    const today = now.toISOString().slice(0, 10);
    const days = (n) => new Date(now.getTime() - n * 86_400_000).toISOString().slice(0, 10);
    const presets = {
...
exports.adsmanagerRouter.get('/accounts', auth_js_1.authenticate, async (req, res) => {
    try {
...
exports.adsmanagerRouter.get('/overview', auth_js_1.authenticate, async (req, res) => {
    try {
        const q = req.query;
        ...
        if (!account) {
            res.status(400).json({ error: 'account requerido' });
            return;
        }
        const token = await getUserToken(req.user.id);
        const insightFields = [
        ...
        const [campaigns, prevCampaigns, campaignMeta] = await Promise.all([
            metaFetchAll(`/${account}/insights`, token, {
                fields: insightFields,
                level: 'campaign',
                time_range: JSON.stringify({ since: range.since, until: range.until }),
                limit: '200',
            }),
            compare ? metaFetchAll(`/${account}/insights`, token, {
                fields: insightFields,
                level: 'campaign',
            ...
            metaFetchAll(`/${account}/campaigns`, token, {
        ...
        for (const row of campaigns) {
            ...
            if (!byId.has(id)) {
                ...
                cur['spend'] = (parseFloat(cur['spend'] ?? '0') + parseFloat(row['spend'] ?? '0'));
                cur['impressions'] = (parseInt(cur['impressions'] ?? '0', 10) + parseInt(row['impressions'] ?? '0', 10));
                cur['clicks'] = (parseInt(cur['clicks'] ?? '0', 10) + parseInt(row['clicks'] ?? '0', 10));
                cur['reach'] = (parseInt(cur['reach'] ?? '0', 10) + parseInt(row['reach'] ?? '0', 10));
                cur['_rows'] = cur['_rows'] + 1;
                // Merge actions
                const curActions = cur['actions'] ?? [];
                const rowActions = row['actions'] ?? [];
                const merged = {};
                for (const a of [...curActions, ...rowActions]) {
                    const k = a['action_type'];
                    merged[k] = (merged[k] ?? 0) + parseFloat(a['value'] ?? '0');
                }
        ...
        const normCampaigns = [...byId.values()].map(row => {
            ...
            const getAction = (types) => actions.filter(a => types.includes(a['action_type'])).reduce((s, a) => s + parseFloat(a['value'] ?? '0'), 0);
            ...
            const revenue = row['purchase_roas']?.[0]
                ? spend * parseFloat(row['purchase_roas'][0].value)
                : 0;
            return {
                campaign_id: campId,
        ...
        if (compare && prevCampaigns.length) {
            const pSpend = prevCampaigns.reduce((s, r) => s + parseFloat(r['spend'] ?? '0'), 0);
            const pImpr = prevCampaigns.reduce((s, r) => s + parseInt(r['impressions'] ?? '0', 10), 0);
            const pClk = prevCampaigns.reduce((s, r) => s + parseInt(r['clicks'] ?? '0', 10), 0);
...
exports.adsmanagerRouter.post('/pause', auth_js_1.authenticate, async (req, res) => {
    try {
        const { id, level = 'CAMPAIGN' } = req.body;
        if (!id) {
...
exports.adsmanagerRouter.post('/activate', auth_js_1.authenticate, async (req, res) => {
    try {
        const { id } = req.body;
        if (!id) {
            res.status(400).json({ error: 'id requerido' });
...
exports.adsmanagerRouter.post('/bulk-action', auth_js_1.authenticate, async (req, res) => {
    try {
        const { ids, action } = req.body;
        if (!ids?.length || !action) {
            res.status(400).json({ error: 'ids y action requeridos' });
            return;
        }
        const token = await getUserToken(req.user.id);
...
exports.adsmanagerRouter.post('/budget', auth_js_1.authenticate, async (req, res) => {
    try {
        const { id, daily_budget } = req.body;
        if (!id || !daily_budget) {
            res.status(400).json({ error: 'id y daily_budget requeridos' });
            return;
        }
        const token = await getUserToken(req.user.id);
...
exports.adsmanagerRouter.get('/top-ads', auth_js_1.authenticate, async (req, res) => {
    try {
        const q = req.query;
        const account = q['account'];
        const date = q['date'] ?? 'last_30d';
        ...
        const ads = await metaFetchAll(`/${account}/insights`, token, {
            fields,
            level: 'ad',
            time_range: JSON.stringify({ since: range.since, until: range.until }),
            limit: '200',
        ...
        for (const row of ads) {
            const id = row['ad_id'];
            if (!byAd.has(id)) {
                byAd.set(id, { ...row, spend: parseFloat(row['spend'] ?? '0') });
            }
            else {
                const cur = byAd.get(id);
                cur['spend'] = cur['spend'] + parseFloat(row['spend'] ?? '0');
                cur['impressions'] = (parseInt(cur['impressions'] ?? '0', 10) + parseInt(row['impressions'] ?? '0', 10));
                cur['clicks'] = (parseInt(cur['clicks'] ?? '0', 10) + parseInt(row['clicks'] ?? '0', 10));
            }
        }
        ...
        const adIds = sorted.map(a => a.ad_id);
        let previews = {};
        ...
        const result = sorted.map(a => ({
            ...a,
            preview_url: previews[a.ad_id]?.preview_shareable_link ?? null,
            thumbnail_url: previews[a.ad_id]?.creative?.image_url ?? previews[a.ad_id]?.creative?.thumbnail_url ?? null,
        }));
...
exports.adsmanagerRouter.get('/countries', auth_js_1.authenticate, async (req, res) => {
    try {
        const q = req.query;
        const account = q['account'];
        const date = q['date'] ?? 'last_30d';
        ...
        const token = await getUserToken(req.user.id);
        const rows = await metaFetchAll(`/${account}/insights`, token, {
        ...
        for (const row of rows) {
            ...
            if (roas)
                cur.revenue += cur.spend * parseFloat(roas.value);
...
exports.adsmanagerRouter.get('/launches', auth_js_1.authenticate, async (req, res) => {
    try {
        const q = req.query;
        const account = q['account'];
        ...
        for (const row of rows) {
            ...
            const cur = byLaunch.get(launchName) ?? {
                name: launchName, campaigns: new Set(), days: new Set(),
                spend: 0, impressions: 0, clicks: 0, purchases: 0, leads: 0, revenue: 0,
            };
            cur.campaigns.add(row['campaign_id']);
            cur.days.add(row['date_start']);
            cur.spend += parseFloat(row['spend'] ?? '0');
            cur.impressions += parseInt(row['impressions'] ?? '0', 10);
            cur.clicks += parseInt(row['clicks'] ?? '0', 10);
            const actions = row['actions'] ?? [];
...
exports.adsmanagerRouter.get('/recommendations', auth_js_1.authenticate, async (req, res) => {
    try {
        const q = req.query;
        const account = q['account'];
        const date = q['date'] ?? 'last_30d';
        ...
        for (const c of campMeta)
            campStatusMap.set(c['id'], c['effective_status']);
        // Aggregate by campaign_id to avoid duplicates from pagination
        const byId = new Map();
        for (const row of rows) {
            const id = row['campaign_id'];
            if (!byId.has(id)) {
                byId.set(id, { ...row, spend: parseFloat(row['spend'] ?? '0') });
            }
            else {
                const cur = byId.get(id);
                cur['spend'] = cur['spend'] + parseFloat(row['spend'] ?? '0');
        ...
        const campaigns = [...byId.values()].map(row => {
            ...
            return {
                ...
                status: campStatusMap.get(row['campaign_id']) ?? 'UNKNOWN',
        ...
        for (const c of campaigns) {
            if (c.spend < 1)
                continue;
            if (c.cpa > 0 && c.cpa > avgCpa * 2) {
                pause.push(c);
                continue;
            }
            if (c.roas > 2 && c.spend > 50) {
                scale.push(c);
                continue;
            }
            if (c.ctr > avgCtr * 1.5 && c.purchases === 0) {
                review.push(c);
                continue;
            }
            ok.push(c);
...
exports.adsmanagerRouter.get('/projections', auth_js_1.authenticate, async (req, res) => {
    try {
        const q = req.query;
        const account = q['account'];
        const date = q['date'] ?? 'last_30d';
...
exports.adsmanagerRouter.get('/config', auth_js_1.authenticate, async (req, res) => {
    try {
        const { data: profile } = await supabase_js_1.supabaseAdmin
            ...
            .single();
        res.json({
            defaultAccount: profile?.meta_account_id ?? process.env.META_AD_ACCOUNT_ID ?? null,
...
exports.adsmanagerRouter.post('/save-token', auth_js_1.authenticate, async (req, res) => {
    try {
        ...
        if (account_id)
            updates['meta_account_id'] = account_id;
        ...
        if (token) {
            try {
                ...
                console.log('[save-token] Token válido, usuario Meta:', check?.name ?? check?.id);
            }
            catch (metaErr) {
        ...
        const { error } = await supabase_js_1.supabaseAdmin.from('users').update(updates).eq('id', req.user.id);
        if (error) {
...
exports.adsmanagerRouter.post('/save-name', auth_js_1.authenticate, async (req, res) => {
    try {
        const { name } = req.body;
        if (!name) {
        ...
        await supabase_js_1.supabaseAdmin.from('users').update({ name }).eq('id', req.user.id);
        res.json({ success: true });
    }
    catch (err) {
        res.status(500).json({ error: err instanceof Error ? err.message : 'Error' });
...
exports.adsmanagerRouter.get('/me', auth_js_1.authenticate, async (req, res) => {
    try {
        const { data: profile } = await supabase_js_1.supabaseAdmin
            .from('users')
            ...
            .eq('id', req.user.id)
            .single();
        await supabase_js_1.supabaseAdmin.from('users').update({ last_login: new Date().toISOString() }).eq('id', req.user.id);
...
// ── GET /api/version ─────────────────────────────────────────────────────────
exports.adsmanagerRouter.get('/version', (_req, res) => {

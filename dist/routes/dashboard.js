...
function mapStageKey(stageName) {
...
async function getFilteredMetaSpend(since, until, campaignIds) {
...
async function getFilteredMetaTotals(since, until, campaignIds) {
    let query = supabase_js_1.supabaseAdmin
        .from('meta_campaigns')
...
exports.dashboardRouter.get('/funnel', auth_js_1.authenticate, async (req, res) => {
    try {
        const city = req.query.city;
        const since = req.query.since;
        const until = req.query.until;
        const pipelineType = req.query.pipeline_type ?? 'registro';
        ...
        if (!since || !until) {
            res.status(400).json({ error: 'Parámetros "since" y "until" requeridos', status: 400 });
            return;
        }
        const { data, error } = await supabase_js_1.supabaseAdmin.rpc('get_funnel_by_city', {
            p_city: city,
            p_since: since,
            p_until: until,
...
exports.dashboardRouter.get('/traffic', auth_js_1.authenticate, async (req, res) => {
    try {
        const since = req.query.since;
        const until = req.query.until;
        ...
        const { data, error } = await supabase_js_1.supabaseAdmin.rpc('get_traffic_kpis', {
            p_since: since,
            p_until: until,
            p_city: city,
        });
        ...
        if (campaignIds.length > 0) {
            ...
            result.meta_cpl = meta.conversions > 0
                ...
                : 0;
            const crmLeads = result.crm_leads ?? 0;
...
exports.dashboardRouter.get('/campaigns', auth_js_1.authenticate, async (req, res) => {
    try {
        const since = req.query.since;
        const until = req.query.until;
...
exports.dashboardRouter.get('/ads-preview', auth_js_1.authenticate, async (req, res) => {
    try {
        const since = req.query.since;
        const until = req.query.until;
        if (!since || !until) {
...
// ?since=YYYY-MM-DD&until=YYYY-MM-DD&city=bucaramanga (city opcional)
exports.dashboardRouter.get('/survey', auth_js_1.authenticate, async (req, res) => {
    try {
        const since = req.query.since;
        const until = req.query.until;
        const city = req.query.city ?? null;
        ...
        const { data, error } = await supabase_js_1.supabaseAdmin.rpc('get_survey_breakdown', {
            p_since: since,
            p_until: until,
            p_city: city,
        });
